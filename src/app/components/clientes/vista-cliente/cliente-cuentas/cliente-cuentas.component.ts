...
@Component({
    ...
    styleUrl: './cliente-cuentas.component.scss'
})
export class ClienteCuentasComponent implements OnInit {
    ...
    // Variable para guardar el nombre completo del cliente
    ...
    ngOnInit() {
        ...
        console.log("ngOnInit", this.idCliente)
    }
    ...
    obtenerCliente(id_cliente: any) {
        this.clientesService.obtenerById(id_cliente).subscribe({
            next: (response: any) => {
                ...
                this.cliente = body[0];
                this.nombreCompleto = this.cliente.nombre_completo
                console.log("obtenerCliente", this.cliente)
                this.obtenerCuentasPorCobrar(this.cliente?.id_persona);
                this.obtenerPagosRecibidos(this.idCliente);
            },
            error: (error: any) => {
    ...
    obtenerPagosRecibidos(idCliente: any): void {
        this.cargandoPagos = true;

        this.pagosRecibidosService.obtenerByCliente(idCliente).subscribe({
            next: (response: any) => {
    ...
    filtrarPagos(): void {
        ...
        if (this.searchTerm && this.searchTerm.trim() !== '') {
            const termino = this.searchTerm.toLowerCase();
            resultado = resultado.filter(item =>
                (item.nombre_representante || '').toLowerCase().includes(termino) ||
    ...
    async exportarPDF(): Promise<void> {
        try {
            ...
            const datosPDF: DatosCuentasPDF = {
                nombreCliente: this.nombreCompleto || 'N/A',
                numeroIdentificacion: this.cliente?.numero_identificacion,
                nombrePlan: this.cliente?.nombre_plan,
    ...
    compartirPorWhatsApp(): void {
        ...
        mensaje += `👨‍🎓 Cliente: ${this.nombreCompleto}\n\n`;
