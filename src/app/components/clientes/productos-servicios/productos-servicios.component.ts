...
export class ProductosServiciosComponent implements OnInit {
  ...
  public idCliente = "0";
  public accion = "";
  public path = "/clientes/productos-servicios/crear/0/"
  public cliente: any;
  ...
  public acciones = [
    { id: 'cambiar_plan', label: 'Cambiar Plan', icono: '/assets/images/editar.png' },
  ...
  configurarPermisos(): void {
    this.puedeAdministrar = this.permisosService.tienePermiso('clientes.productos_servicios.administrar');
  }
  ...
  obtenerCliente(id_cliente: any) {
    this.clientesService.obtenerById(id_cliente).subscribe((response: any) => {
      ...
      this.obtenerTodos(this.cliente.id_persona);
  ...
  seleccionar(event: any) {
    if (event.accion === 'editar') {
      this.router.navigate(['clientes/productos-servicios/editar/' + event.id + '/' + this.idCliente]);
    }
    ...
    if (event.accion === 'consultar') {
      this.router.navigate(['clientes/productos-servicios/consultar/' + event.id + '/' + this.idCliente]);
    }
  }
