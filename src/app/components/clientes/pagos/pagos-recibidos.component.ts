...
export class PagosRecibidosComponent implements OnInit {
  ...
  public idCliente = "0";
  public accion = "";
  public path = "/clientes/pagos/crear/0/"
  public cliente: any;
  ...
  configurarPermisos(): void {
    this.puedeAdministrar = this.permisosService.tienePermiso('clientes.pagos.administrar');
  }

  obtenerByCliente(): void {
    this.pagosRecibidosService.obtenerByCliente(this.idCliente).subscribe((response: any) => {
  ...
  seleccionar(event: any) {
    ...
    if (event.accion === 'editar') {
      this.router.navigate(['clientes/pagos/editar/' + event.id + '/' + this.idCliente]);
    }
    else if (event.accion === 'eliminar') {
      this.anularPago(event.id, event.registro);
    }
    else if (event.accion === 'consultar') {
      this.router.navigate(['clientes/pagos/consultar/' + event.id + '/' + this.idCliente]);
  ...
  imprimir(id: any, registro: any) {
    ...
    // Importante: Aquí debes usar el ID del pago, NO el ID del cliente
