...
export class MigracionRegistrosService {

  private servicio = environment.api + 'migracion-registros';

  constructor(private http: HttpClient) {}

  obtenerTodos(idSesion: string, tablaDestino?: string) {
    return this.http
      .get<HttpResponse<Object>>(tablaDestino ? `${this.servicio}?id_sesion=${idSesion}&tabla_destino=${tablaDestino}` : `${this.servicio}?id_sesion=${idSesion}`, { observe: 'response' })
      .pipe(
  ...
  private handleError(error: HttpErrorResponse): Observable<never> {
    console.error('Error en MigracionRegistrosService:', error);
