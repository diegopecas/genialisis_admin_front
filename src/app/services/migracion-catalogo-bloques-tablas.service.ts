...
export class MigracionCatalogoBloquesTablasService {

  private servicio = environment.api + 'migracion-catalogo-bloques-tablas';

  constructor(private http: HttpClient) {}

  obtenerTodos(idCatalogoBloque?: string) {
    return this.http
      .get<HttpResponse<Object>>(idCatalogoBloque ? `${this.servicio}?id_catalogo_bloque=${idCatalogoBloque}` : this.servicio, { observe: 'response' })
      .pipe(
  ...
  private handleError(error: HttpErrorResponse): Observable<never> {
    console.error('Error en MigracionCatalogoBloquesTablasService:', error);
