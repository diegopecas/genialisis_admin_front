...
export class MigracionBloquesService {

  private servicio = environment.api + 'migracion-bloques';
  ...
  /** Se marca validado después de revisar en pantalla que los datos quedaron bien. */
  validar(id: string): Observable<any> {
    var body = JSON.stringify({ id });
    return this.http.post<any>(`${this.servicio}/validar`, body, httpOptions).pipe(
      catchError(this.handleError)
    );
  }

  private handleError(error: HttpErrorResponse): Observable<never> {
    console.error('Error en MigracionBloquesService:', error);
