...
export class ConceptosFinancierosService {
  private servicio = environment.api + 'conceptos-financieros';
  ...
  obtenerPorCategoria(idCategoria: number) {
    return this.http
      .get<HttpResponse<Object>>(`${this.servicio}/categoria/${idCategoria}`, { observe: 'response' })
      .pipe(
        tap((response: HttpResponse<Object>) => {
          let respuesta: any = response.body;
          if (respuesta.error) {
  ...
  obtenerPorTipoMovimiento(idTipo: number) {
    return this.http
