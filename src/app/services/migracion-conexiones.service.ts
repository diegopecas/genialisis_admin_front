...
export class MigracionConexionesService {

  private servicio = environment.api + 'migracion-conexiones';
  ...
  /** Confirma que conecta y devuelve qué se encontró en la base destino. */
  probar(id: string): Observable<any> {
    var body = JSON.stringify({ id });
    return this.http.post<any>(`${this.servicio}/probar`, body, httpOptions).pipe(
      catchError(this.handleError)
    );
  }

  private handleError(error: HttpErrorResponse): Observable<never> {
    console.error('Error en MigracionConexionesService:', error);
