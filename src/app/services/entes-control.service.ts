...
export class EntesControlService {
  private servicio = environment.api + 'entes-control';
  ...
  verificarDuplicados(idPersona: string) {
    return this.http
      .get<HttpResponse<Object>>(
        environment.api + `entes-control-duplicados/${idPersona}`,
        { observe: 'response' }
      )
      .pipe(
        tap((response: HttpResponse<Object>) => {
